...
if script.vars[1] =~ /help/
	...
	if CharSettings[:jars].nil? || script.vars[1]=~/newlocker/
		...
		jars.each { |jar|
			echo "Moving all #{jar}s from your harness into your locker"
			output="You put"
			while output =~ /You put/ do
				dothistimeout "get #{jar} from harness", 3, /^You remove|Get what/
				output = dothistimeout "put my #{jar} in locker", 3, /^You put|^You place|I could not find/
			end
		}
		jars.each { |jar|
			output="You put"
			...
			while output =~ /You put/ do
				dothistimeout "get #{jar} from harness", 3, /^You remove|Get what/
				output = dothistimeout "put my #{jar} in locker", 3, /^You put|I could not find|^You place/
			end
		}
	end
	...
	dothistimeout "close locker", 1, /^You close|^That is already closed|^You hear the faint\.$/
	status_tags
	open_result = dothistimeout 'open locker', 5, /exist=".*?" noun=".+"/
	status_tags

	if open_result =~ /exist="(\d+)" noun=".+"/
		fput "location"
		...
		if locker_contents
			if CharSettings[:jars].nil?
				...
				while broken==true do
					...
					dothistimeout "close locker", 1, /^You close|^That is already closed|^You hear the faint\.$/
					status_tags
					open_result = dothistimeout 'open locker', 5, /exist="(\d+)" noun=".+"/
					locker_id = $1
					status_tags
					if open_result =~ /exist="(\d+)" noun=".+"/
						locker_id = $1
...
####
			if script.vars[1]=~/raid/	
				...
				gem_wardrobe.each { |gem|
					...
					dothistimeout "close locker", 1, /^You close|^That is already closed|^You hear the faint\.$/
					status_tags
					open_result = dothistimeout 'open locker', 5, /exist=".*?" noun="(locker)"|already open/
					status_tags
								...
								if jar = locker_contents.find { |jar| jar.after_name.gsub(/containing |large |medium |small |tiny |some /, '') =~ /^#{gem.gsub(/large |medium |small |tiny |some /, '').sub(/y\b/, '(?:y|ie)').sub(/z\b/, 'ze?').gsub(/\b\s/, 's? ')}s?$/ }
									...
									if jar_result=~/^You remove/ then
										...
										if jar_hash[:count] < 1
											...
											dothistimeout "put ##{jar.id} in locker", 3, /^You (?:put|place)/
										end
										dothistimeout "put ##{jar.id} in locker", 3, /^You (?:put|place)/
						...
						gemsack.contents.each { |obj|
							if obj.name.gsub(/large |medium |small |tiny |some /, '') == gem_name and not squelch.include?(gem_name)
								...
								if result =~ /^You add .* filling it/
									jar_hash[:count] = jar_hash[:count] + 1
									jar_hash[:full] = true
									dothistimeout "put ##{jar.id} in locker", 3, /^You put|^You place/
									break
									...
									dothistimeout "put ##{obj.id} in ##{gemsack.id}", 3, /^You put/
									jar_hash[:full] = true
									dothistimeout "put ##{jar.id} in locker", 3, /^You put/
									break
								elsif result =~ /^You add/
									jar_hash[:count] = jar_hash[:count] + 1
								elsif result =~ /does not appear to be a suitable container for/
