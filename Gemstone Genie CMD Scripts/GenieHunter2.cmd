...
action put #queue clear; send 1 $lastcommand when ^You are still stunned
...
var OPTIONVARS AMB.*?|APPR.*?|ARRA.*?|BACK.*?|BAST.*?|BLO.*?|BRA.*?|BS|BUN.*?|COLL.*?|COUNT|CUST.*?|DANCE|DEF.*?|DMSET|DMULTI|DODGE|DSET|DYING|EMP.*?|EVA.*?|EXP|HELP|HUNT|JUGG.*?|JUNK|LOOTA.*?|LOOTB.*?|LOOTC.*?|LOOTG.*?|MAGIC|MSET|MULTI|NOEV.*?|NOPA.*?|NOSH.*?|OFF.*?|PARRY|PM|POACH|POW.*?|PP|RET.*?|ROAM|SCRAPE.*?|SKIN.*?|SKINRET.*?|SLOW|SNAP|SNIP.*?|STACK|STANCE|SWAP|TARG.*?|THROW|TIME.*?|TM|TRAIN|TSWAP|YOYO
var OPTION NONE
...
var BOX_CONTAINER bag
var GEM_CONTAINER bag
var JUNK_CONTAINER bag
...
SHEATHE:
	...
	SHEATHING:
			...
			matchre RETURN ^You sheath|^You strap|^With a flick of your wrist|^You hang|you sheath|^Sheathe what\?|^You easily strap|^With fluid and stealthy movements|^You slip
		put shea %COMM_STRING
		matchwait 6
	WEAR_WEAPON:
			match STOW_WEAPON You can't wear that!
			matchre RETURN You sling|Wear what?
...
GENERAL_TRIGGERS:
	...
	action goto STAND when You should stand up first
...
COMBAT_COMMAND:
	...
	if ((toupper("$GH_HUNT") = "ON") && (%t >= %HUNT_TIME) && ($Perception.LearningRate < 30) && ($Stalking.LearningRate < 30)) then
	{
		put hunt
		var HUNT_TIME %t
	...
	if ((toupper("$GH_PP") = "ON") && (%t >= %PP_TIME) && ($Power_Perceive.LearningRate < 30)) then
	{
		put perc
		var PP_TIME %t
...
WEBBED:
	...
	action var WEBBED NO when move freely again
	put #beep
	if ("%WEBBED" = "YES") then 
	{
...
STOW_GEM:
	var LAST STOW_GEM		
		matchre NO_MORE_ROOM_GEM any more room in|^Stow what|^You just can't get|^But that's closed
		matchre LOOT_GEM_ON ^You put your
	put stow gem
	matchwait 30
	goto LOOT_ERROR
...
HEALTH_CHECK:
	gosub clear
		match HEALTH_CHECK pole|still stunned
