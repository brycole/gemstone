...
var boxtype cracked|rusty|reinforced|worm-eaten|dull|dented|deeply gouged|copper-edged|plain|crooked|mud-stained|pitted|salt-stained|sturdy|rotting|moldy|misshaped|poorly made|crust-covered|mildewy|corroded|greenish brass|copper|deobar|driftwood|iron|ironwood|mahogany|oaken|pine|steel|wooden
var boxes coffer|crate|strongbox|caddy|casket|skippet|trunk|chest|\bbox
var junkloot scroll|tablet|vellum|sheiska leaf|ostracon|bark|papyrus roll|smudged parchment|fragment|dira|tattered papyrus|seishaka leaf|card
...
var skinnablemonsters1 frog|silverfish|musk hog|grub|crayfish|burrower|crab|boar|skunk|badger|pothanit|trollkin|fell hog
...
var skinnablemonsters5 la'heke|vykathi soldier|boa|warcat|moda|arzumos|carcal|blood warrior|goblin|cave troll
...
var skinnableundead2 zombie kobold savage|zombie kobold headhunter|ghoul|misshapen germish'din
var skinnableundead1 squirrel|grendel|reaver|mey|shadow hound|lach|mastiff|gremlin

var invasioncritters transmogrified oaf|flea-ridden beast|Prydaen|Rakash|bone warrior|shambling horror|skeletal peon|revivified mutt
...
var nonskinnablecritters %monsters1|%monsters2|%monsters3|%monsters4|%monsters5|%monsters6|%monsters7|%undead1|%undead2|%undead3

var critters %nonskinnablecritters|%skinnablecritters|%invasioncritters

var OPTIONVARS AMB.*?|APPR.*?|BS|BACK.*?|BLO.*?|BUN.*?|BRA.*?|DAN.*?|COU.*?|EMP.*?|DOD.*?|EVA.*?|EXP|HUNT|LOOTA.*?|LOOTB.*?|LOOTC.*?|LOOTG.*?|OFF.*?|PAR.*?|PO.*?|RET.*?|SKINR.*?|SKIN.*?|SNA.*?|SNI.*?|STAN.*?|THR.*?|TIM.*?|TM|PM|MAG.*?|TRAIN|TARG.*?|CUS.*?|DSET|DEF.*?|MSET|MU.*?|DMSET|DMU.*?|SWAP|BAST.*?|TSWAP|JUNK.*?|ROAM|ARRA.*?|HELP|SLOW|NOE.*?|NOP.*?|NOS.*?|SCRAP.*?|JUGG.*?|YOYO|DYING
var OPTION NONE
...
var SLING_AMMO rock
var QUIVER basket
var LT_SHEATH weapon.belt
var HT_SHEATH war.belt
var BOX_CONTAINER pack
var GEM_CONTAINER bag
var JUNK_CONTAINER basket
...
action goto calm when you don't feel like fighting right
action var guild $1 when Guild: (\S+)
action var level $1 when Circle: (\d+)
put #script pause afkhunt
put info 
waitfor Encumbrance
...
ARRANGE:
	...
	if ("$GH_SKIN" != "ON") then put #var GH_SKIN ON
	if matchre("%1","^\d+$") then
	{
		put #var MAX_ARRANGE %1
		echo *** Arranging %1 times ***
		shift
...
BACKSTAB:
	...
	APPRAISE_BS:
		...
		if $monstercount > $maxcrit then put retreat
...
MSET:
	...
	var SET_NUM 1
	SET_M_STRING:
		if (SET_NUM > 10) then goto DONE_M_SET
...
WEAPON_CHECK:
	...
	2_HANDED_WEAPON:
		if ("%MAGIC_TYPE" != "OFF") then var MAGIC_COUNT %c
		if ("%DSET" = "ON") then goto SET_DEFAULT
		if $GH_DANCING = ON then goto count_wait_1
		goto %c
...
WIELD_WEAPON:
	...
	echo
	WIELDING_WEAPON:
			matchre REMOVING_WEAPON out of reach|You'll need to remove|What were you|can't seem|is out of reach
			matchre RETURN You draw|already holding|You deftly remove|With a flick of your wrist|With fluid and stealthy movements
		put wield my %STRING
		matchwait 6
	REMOVING_WEAPON:
...
SHEATHE:
	...
	SHEATHING:
			...
			matchre RETURN You sheath|You strap|With a flick of your wrist|You hang|you sheath|Sheathe what\?|You easily strap|With fluid and stealthy movements
		if $monstercount > $maxcrit then put retreat
...
#####################################
GENERAL_TRIGGERS:
	action goto STAND when eval $standing = 0|You should stand up first
	...
	action goto ABORT when eval $health < 40
...
RETREAT_TRIGGERS:
	var RETREATING ON
	var RETREATED NO
	action if ("%RETREATED" = "NO") then send ret;var RETREATED YES when closes to pole weapon range
MELEE_TRIGGERS:
	action if ("%RETREATED" = "NO") then send ret;send ret;var RETREATED YES when (^\*.*at you\..*You)|(closes to melee range on you)
...
ADVANCE:
	...
	ADV_NOW:
		...
		var LAST ADV_NOW
			...
			match COUNT_$GH_DANCING already at melee
		put advance
		matchwait 30
		goto ERROR
	ADVANCING:
		if ("$GH_DANCING" = "ON") then goto COUNT_$GH_DANCING
		if ("%RANGED" = "ON") then goto %c
		##else waitforre to melee range|\[You're|stops you|thrums
		goto %c

APPR_YES:
	if $gh != gen.2 then 
	{
	if (($Evasion.LearningRate < 1) || ($Shield_Usage.LearningRate < 5) || ($Leather_Armor.LearningRate < 1)) then
	{
		echo *** Evasion is not learning, switching to count mode ***
		put #var gh gen.2
		put gen2
	}
	}
	if ("%APPRAISED" = "YES") then goto COUNT_$GH_DANCING
	else var APPRAISED YES
	if ($Appraisal.LearningRate > 33) then goto APPR_NO
	echo
	echo *** APPR_YES: ***
	echo
	...
	APPRAISING:
		...
		if $monstercount > $maxcrit then put retreat
		if (($Mechanical_Lore.LearningRate < 10) && ($link = 1)) then send .link
...
APPR_NO: 
	var lastdirection none
	if ("%RANGED" != "ON") then goto ADV_NOW
	else goto %c
...
COUNT_START:
	...
	echo *** $monstercount creatures. ***
	if $monstercount > 100 then 
	{
		put #var GH_DANCING OFF
		delay 1
		echo *** Too many creatures in for count! ***
		put stance set 79 1 100 100
		put #var maxcrit 1
		goto %c
	}
	goto COUNT_WAIT_1
...
FACE:      
	...
	echo
	var LAST FACE
		matchre APPR_$GH_APPR You turn to face|You're
...
NO_MONSTERS:
	...
	if ("%THROWN" = "ON") then
	{
		if (matchre("$roomobjs", "%CURR_WEAPON\b")) then put get %CURR_WEAPON
	}
	if ((toupper("$GH_JUGGLE") = "ON") && ($Perception.LearningRate <= 9)) then goto JUGGLE_STUFF
...
JUGGLE_STUFF:
	...
	JUGGLING:
		...
		goto JUGGLE_ERROR	
	JUGGLE_EXP: 
		if $MindState > 2 then goto JUGGLE_STOP
		if $Perception.LearningRate > 9 then goto JUGGLE_STOP
		goto JUGGLING
	JUGGLE_ERROR:
		echo *** Juggling error ***
		put #var GH_JUGGLE OFF
...
COUNT_WAIT_1:
	...
	##if (($Mechanical_Lore.LearningRate < 10) && ($link = 1)) then
	##	{
	##		send .link
	##		exit
	##	}
	...
	{
		echo *** Evasion is learning! Switching to roam mode. ***
		##put gen1
		##put #var gh gen.1
		send .link
		exit
...
400:
	...
	counter set 400
	save 400
	if ("%lastmaneuver" = "%COMBO4") then gosub COMBAT_COMMAND %COMBO1 %HAND $GH_TARGET
...
500:
	...
	counter set 500
	save 500
	if ("%lastmaneuver" = "%COMBO5") then gosub COMBAT_COMMAND %COMBO1 %HAND $GH_TARGET
	if ("%lastmaneuver" = "%COMBO1") then gosub COMBAT_COMMAND %COMBO2 %HAND $GH_TARGET
	if ("%lastmaneuver" = "%COMBO2") then gosub COMBAT_COMMAND %COMBO3 %HAND $GH_TARGET
	if ("%lastmaneuver" = "%COMBO3") then gosub COMBAT_COMMAND %COMBO4 %HAND $GH_TARGET
...
LOAD:
		matchre RANGE_GET You must|your hand jams|readily available|You can not load
...
GET_THROWN:
		...
		matchre RETURN already|You need a free hand to pick that up|You get|You pick up|With a flick|You pull
	put get %CURR_WEAPON
	matchwait 30
	goto THROWN_ERROR
...
COMBAT_COMMAND:
	...
	if (toupper("$GH_HUNT") = "ON") then
	{
		if (($Perception.LearningRate < 11) && (%t >= %HUNT_TIME)) then
		{
				if $monstercount > $maxcrit then 
				{
					put retreat
	...
	if ((toupper("$GH_AMBUSH") = "ON") && ($hidden != 1)) then
	{
		...
		else if (($Hiding.LearningRate < $Halberds.LearningRate) || ($Hiding.LearningRate < 20)) then gosub HIDE
	}
	}
	if (($hidden = 1) && ("%guild" = "Paladin")) then send unhid
	##if ($hidden = 1) then put stop stalk
	if (toupper("$GH_SLOW") = "ON") then pause 3
ATTACK:
	if ($monstercount > $maxcrit) then goto retreat1
	command1:
		...
		matchre RETURN You are already in a position|But you are already dodging|You move into a position to|Roundtime|must be hidden|are not hidden|What were you|What are you
	put %COMMAND
	matchwait 30
	goto ATTACK_ERROR

retreat1:
match retreat2 melee
match retreat2 body thrums with the sudden pressure 
match retreat2 You're
matchwait 10
retreat2:
put retreat
match command1 You retreat
match command1 You are already
match command1 You stop advancing
match command1 You sneak back out
match retreat2 Roundtime
matchwait 20
goto ATTACK_ERROR
...
HIDE:
	if $monstercount <= $maxcrit.hide then 
	{
	...
	if $monstercount > $maxcrit then put advance
		matchre HIDE fail|discovers
		matchre HIDE1_RETREAT You are too close|notices|reveals
		matchre HIDEADV You melt|You blend|Eh\?
	put hide
	matchwait 30
	goto STEALTH_ERROR
	}
...
HIDEADV:
matchre pole You close to pole
matchre pole to melee range|\[You're|stops you|at melee
put advance
matchwait 10
return

pole:
if $hidden = 1 then put stalk
pause
if %hide.a = 1 then
{
put retreat
...
STALK1:
		...
		matchre RETURN You move into position|You are already stalking
	put stalk
	matchwait 30
	goto STEALTH_ERROR
...
MAGIC_TARGET:
	...
	echo MAGIC_TARGET:
	echo
		match MAGIC_PREP You don't have a spell prepared!
...
MAGIC_TARGET_WAIT1:
	...
	if (("$GH_SNAP" = "ON") || ("%FULL_PREP" = "YES")) then goto MAGIC_CAST
...
MAGIC_TARGET_WAIT2:
	...
	if ("$GH_SNAP" = "ON") then goto MAGIC_CAST
	goto MAGIC_CAST
...
MAGIC_FACE_TM:
	...
	echo
		...
		match MAGIC_TM_ADV There is nothing
	put face next
	matchwait 30
	goto MAGIC_ERROR
...
calm:
echo *** Calmed ***
if $monstercount > 3 then send .link
delay 5
send .counthide
waitfor Roundtime
goto count_wait_1
...
WEBBED:
	...
	put #beep
	waitforre move freely again|manage to free
	action goto WEBBED when eval $webbed = 1
	goto %LAST
...
DEAD_MONSTER:
	...
	if ("%BACKSTAB" = "ON") then counter set 1300
	if ("%RETREATING" = "ON") then gosub RETREAT_UNTRIGGERS
	if ("%THROWN" = "ON") then
	{
		if ((("%HAND" = "") && ("$righthand" = "Empty")) || (("%HAND" = "left") && ("$lefthand" = "Empty"))) then gosub GET_THROWN
	...
	else if matchre ("$roomobjs", "(%nonskinnablecritters) ((which|that) appears dead|\(dead\))") then goto SEAR_MONSTER
	else if matchre ("$roomobjs", "(%invasioncritters) ((which|that) appears dead|\(dead\))") then goto SEAR_MONSTER
	goto NO_MONSTER
...
SKIN_MONSTER:
	...
	var LAST SKINNING
			...
			matchre SKIN_CHECK Roundtime|You must have one hand free to skin.
		if $monstercount > $maxcrit then put retreat
		put skin
		matchwait 30
		goto SKIN_ERROR
...
ARRANGING:
	...
	var LAST ARRANGING
	ARRANGEIT:
			matchre SKIN_FAIL corpse is worthless now|Arrange what|^You might want to kill it first
...
SCRAPE_ON:
	...
	var LAST SCRAPE_ON
	put sear %Monster
	if ($Mechanical_Lore.LearningRate > 25) then goto SCRAPE_OFF
...
SEAR_MONSTER: 
	var Monster $1
SKIN_REEQUIP_DONE: 
	if (($monstercount <= %xCOUNT) && ($gh = gen.2)) then
	{
		delay 1
		echo *** Switching back to count mode ***
		put stance set 100 1 99 0
		put #var maxcrit 2
		put #var GH_DANCING ON
	}
	pause 1
...
LOOT_COIN_ON:
	if matchre ("$roomobjs", "(silver coin)") then goto GET_COIN
LOOT_COIN_OFF:
...
STOW_BOX:
	var LAST STOW_BOX
		matchre NO_MORE_ROOM_BOX any more room in|Stow what|You just can't get
		match LOOT_BOX_ON You put your
	put stow box
	matchwait 30
	goto LOOT_ERROR
...
GET_GEM:
	var GEM $1
	var LOOTED YES
	if ("%GEM" = "malachite stone") then
	{
		var GEM stone
	}
STOW_GEM:
	var LAST STOW_GEM		
		matchre NO_MORE_ROOM_GEM any more room in|Stow what|You just can't get
		match LOOT_GEM_ON You put your
	put stow gem
	matchwait 30
	goto LOOT_ERROR

NO_MORE_ROOM_GEM:
	if matchre ("$righthandnoun", "(%gems1|%gems2|%gems3|%gems4)") then put empty right
	else put empty left
...
LOOT_DONE:
	var LAST LOOT_DONE
	if ("%STACK" = "ON") then gosub THROWN_CLEAN
	if ("%RANGED" = "ON") then 
...
EXPCHECK_ON:
	...
	if ("%REST" = "OFF") then
	{
		if ($MindState > 2) then goto MIND_MURKED
		if ("%ALTEXP" = "ON") then 
		{
			if ($%EXP2.LearningRate > 7) then
			{
				if ($%EXP2.LearningRate > 10) then goto SWITCH_WEAPON
				else if ($%WEAPON_EXP.LearningRate > 10) then goto SWITCH_WEAPON
			}
		} else if ($%WEAPON_EXP.LearningRate > 10) then goto SWITCH_WEAPON
	}

EXPCHECK_OFF:
	var LAST EXP_CHECK_OFF
	if (toupper("$GH_STANCE") = "ON")) then gosub SWITCH_STANCE
	...
	put #statusbar 2 GH - Kills:$GH_KILLS  Skins:$GH_SKINS  Loots:$GH_LOOTS
...
BLEEDING:
	...
	action var INFECTED YES when wounds are infected
	var INFECTED NO
	put heal
	goto HEALTH_CHECK
...
HEALTH_CHECK:
	...
	matchwait 10
CHECKING_HEALTH:
	if ("%INFECTED" = "YES") then goto DEAD
	if ($stamina >= 60) then
	{
		echo
...
HELP_DEF:
	...
	echo ** 
	echo ** SYNTAX: .geniehunter DSET {weapon set-up}
	echo ** EXAMPLE: .geniehunter DSET loot skin scimtar shield
	echo ** USAGE: .geniehunter
	echo ** 
...
HELP_SKIN:
	...
	echo ** BUNDLE   : Same as 'SKIN', but bundles the skins.  If no ropes to bundle, drops skins
