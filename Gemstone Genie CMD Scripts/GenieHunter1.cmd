...
var junkloot runestone|scroll|tablet|card|vellum|sheiska leaf|ostracon|hhr'lav'geluhh bark|papyrus roll|smudged parchment|lockpick|fragment
...
var skinnablemonsters5 la'heke|vykathi soldier|boa|warcat|moda|arzumo|carcal|blood warrior|goblin|cave troll
...
var invasioncritters transmogrified oaf|flea-ridden beast|Prydean|Rakash|bone warrior|shambling horror|skeletal peon|revivified mutt
...
MELEE_TRIGGERS:
	action if ("%RETREATED" = "NO") then send ret;send ret;var RETREATED YES when (^\*.*at you\..*You)|(closes to pole range on you)
...
RETREAT_UNTRIGGERS:
	var RETREATING OFF
	action remove (^\*.*at you\..*You)|(closes to pole range on you)
...
ADVANCE:
	...
	ADVANCING:
		...
		if (("$GH_AMBUSH" = "ON") ||  ("%RANGED" = "ON")) then goto %c
		else waitforre to pole range|\[You're
		put retreat
		goto %c
...
APPR_YES:
	...
	var LAST APPR_YES
	if ($Appraisal.LearningRate > 10) then goto APPR_NO
	echo
	echo *** APPR_YES: ***
	echo
...
COUNT_START:
	...
	if (($MindState >= 2) || ($Brawling.LearningRate > 10)) then goto COUNT_OFF
	echo
	echo *** COUNT_START: ***
	echo 
	goto %xCOUNT
	...
	COUNT_ONE:
		match %c You notice two
	COUNT_TWO:
		match %c You notice three
	COUNT_THREE:
		match %c You notice four
	COUNT_FOUR:
		match %c You notice five
	COUNT_FIVE:
		match %c You notice six
	COUNT_SIX:
		match %c You notice seven
		match COUNT_WAIT_1 You notice no 
		match COUNT_WAIT_1 Roundtime
	put count critter
	matchwait 30
	goto ERROR
...
JUGGLE_STUFF:
	...
	action goto JUGGLE_STOP when ^\*.*at you\..*You|begins to advance on you|to pole range|to polee range|You're|already at melee	
	JUGGLE_TOP:
		var LAST JUGGLE_TOP
	...
	JUGGLE_STOP:
		...
		action remove ^\*.*at you\..*You|begins to advance on you|to pole range|to polee range|You're|already at melee
...
COUNT_ASSESS_ADV:
	if (($hidden = 1) && ("%GUILD" = "Paladin")) then put unhid
		...
		match NO_MONSTERS advance towards?
		matchre APPR_$GH_APPR begin|to pole range|\[You're|already at melee/
	put advance
	matchwait 10
...
COUNT_ADV:
	...
	goto ERROR	
COUNT_ADVANCING:
		waitforre to pole range|\[You're
		goto %LAST
...
400:
	...
	counter set 400
	save 400
	if ("%lastmaneuver" = "%COMBO4") then gosub COMBAT_COMMAND %COMBO1
...
1220:
	...
	gosub clear
	put adv
	waitforre to pole range|already at melee
...
COMBAT_COMMAND:
	...
	{
		if ((($Perception.LearningRate <= 10) || ($Stalking.LearningRate <= 10)) && (%t >= %HUNT_TIME) && ($MindState < 2)) then
		{
			put hunt
			var HUNT_TIME %t
			math HUNT_TIME add 90
			waitfor Roundtime
		}
	}
	if ((toupper("$GH_AMBUSH") = "ON") && ($hidden != 1)) then
	{
		...
		else if ($Stalking.LearningRate <= 10) then gosub HIDE
	}
...
MAGIC_TM_ADV:
	var LAST MAGIC_TM_ADV
		...
		matchre MAGIC_TARGET to pole range|\[You're|already at melee|begin
	put ADV
	matchwait 30
	goto MAGIC_ERROR
...
SKIN_MONSTER_ON:
	var Monster $1
	if ($Skinning.LearningRate > 10) then goto SKIN_MONSTER_OFF
...
SCRAPE_ON:
	...
	var LAST SCRAPE_ON
	put sear %Monster
	if ($Mechanical_Lore.LearningRate > 10) then goto SCRAPE_OFF
...
GET_ROPE_CONT:
	...
	goto SKIN_REEQUIP	
HAVE_ONE:
	if ("$righthandnoun" = "blade") then gosub SHEATHE blade
	if ("$lefthand" = "Empty") then goto SKIN_ERROR
	##put get bundle
	##put remove bundle
	##waitforre You pick|You sling|You remove
		matchre TOO_HEAVY Time to start a new bundle|You try to stuff your
		matchre HAVE_ONE_CONT You stuff|not going to work
	put bundle
	matchwait 30
	goto SKIN_ERROR
TOO_HEAVY:
