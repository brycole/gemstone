...
## ** Communes	
	...
	var Warding Sanyu Lyba|20|38|9|Warding
	var Augmentation Sanctify Pattern|10|15|21|Augmentation
	var Utility Divine Radiance|10|15|21|Utility
	var Debilitation Soul Sickness|5|5|10|Debilitation
	#var Debilitation Phelim's Sanction|10|10|5|Debilitation
	var Sorcery Clear Vision|15|40|21|Sorcery
	
	## ** 2% - spell prep, 3% - total spell cast, 4% - wait time until spell, 5% - type of spell
	## ** These spells are using the chaos symbiosis
	
	var training_spell_list Sanctify Pattern|Divine Radiance
	...
	var DO_spell_list Aura Sight|Clear Vision|Telekinetic Throw
	var sorc_list Aura Sight|Clear Vision|Telekinetic Throw
...
buffcheck:
	...
	if "%Major" = "off" then gosub castbuff "Major Physical Protection" 15 45 28
	if "%Minor" = "off" then gosub castbuff "Minor Physical Protection" 15 45 28
	if "%Protection" = "off" then gosub castbuff "Protection from Evil" 15 45 28
	if "%Centering" = "off" then gosub castbuff "Centering" 15 45 21
	#if "%Auspice" = "off" then gosub castbuff "Auspice" 10 55 20	
	if "%Shield" = "off" then gosub castbuff "Shield of Light" 15 40 21
	if "%Benediction" = "off" then gosub castbuff "Benediction" 15 35 21
...
pomcast:
	...
	if "$preparedspell" != "None" then gosub release
	gosub castbuff "Persistence of Mana" 375 375 40
...
ritualaltar:
	...
	gosub stow bottle
	var playsong on
	gosub put.1 "play hymn on zill for kertigen" "^You begin"
