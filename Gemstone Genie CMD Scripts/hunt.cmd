########################################################
#Version 15.75 (3/30/2013)
#Edited by the player of Isharon to work with Combat 3.0
#Other major contributors to Geniehunter 3.0 include Azarael and Londrin.
...
var OPTIONVARS AMB.*?|ANAL.*?|APPR.*?|ARM.*?|ARRA.*?|BACKS.*?|BAST.*?|BLO.*?|BON.*?|BRA.*?|BS|BUF.*?|BUN.*?|COLL.*?|COUNT|CUST.*?|DANCE|DANG.*?|DEF.*?|DMSET|DMULTI|DODGE|DSET|DYING|EMP.*?|EVA.*?|EXP|FEINT|HELP|HUNT|JUGG.*?|JUNK|KHRI|LOOTA.*?|LOOTB.*?|LOOTC.*?|LOOTG.*?|MAGIC|MANIP|MARK|MSET|MULTI|NECROH.*?|NECROR.*?|NOEV.*?|NOPA.*?|NOSH.*?|OFF.*?|PARA.*?|PARRY|PART\b|PM|POACH|POUCH|POW.*?|PP|RET.*?|ROAM|SCRAPE.*?|SEARCH|SKIN.*?|SKINRET.*?|SLOW|SNAP|SNIP.*?|STACK|STANCE|SWAP|TAC.*?|TARG.*?|TEND|THIEF|THROW\b|TIE|TIME.*?|TM|TRAIN|TSWAP|WEAR|YOYO
var OPTION NONE
...
action var COMBO $1 when by landing (.*)\.$
put info
waitforre ^Debt:$
put awaken
...
BEGIN:
	...
	if ("%GAG_ECHO" != "YES") then
	{
		...
		matchre WEAPON_CHECK You draw|already holding|free to|With a flick of your wrist
...
DANGER:
	...
	if toupper("$GH_BUFF") != "YES") then gosub m.inputUFF
	return
...
SKIN:
	...
	var BELT_WORN OFF
	action var BELT_WORN ON when ^You tap a(.+)[belt knife|ritualist's knife|wrist knife](.*)you are wearing
	put tap belt knife
	wait
	put tap ritualist's knife
	pause 1
	put tap wrist knife
	pause 1
	action remove ^You tap a(.*)[belt knife|ritualist's knife|wrist knife](.*)you are wearing
	put #var GH_SKIN ON
...
TIE:
	...
	put #var GH_TIE ON
	if ("$GH_BUNDLE" != "ON") then gosub m.inputUNDLE
	return
...
WEAPON_CHECK:
	...
	ME:
		...
		var WEAPON_EXP Small_Edged
		var RANGED OFF
		if matchre("$righthand","adze|axe|cleaver|curlade|cutlass|hanger|hatchet|lata'oloh|mallet|mirror blade|nimsha|parang|riste|sapara|scimitar|scythe-blade|shotel|sword|tei'oloh'ata|telo") then goto ME_SLICE
		if matchre("$righthand","blade|foil|gladius|nambeli|pasabas|rapier|saber|sabre|shashqa|yataghan") then goto ME_STAB
...
GENERAL_TRIGGERS:
	...
	action (hunt) goto STAND when ^You should stand up first|^You had better stand up first
...
APPR_YES:
	if (("%APPRAISED" = "YES") || ($Appraisal.LearningRate >= 34)) then goto COUNT_$GH_DANCING
...
KHRI.START:
	...
	matchre return ^Focusing intently|Roundtime
...
AMBUSH.DECIDE:
	pause 0.1
	if matchre("$righthandnoun","(bola|club|hhr'ata|bludgeon|maul|mace|mattock|star|cane|staff|ngalio|nightstick|khuj)") then var ambush.type stun
...
COMBAT_COMMAND:
	...
	if ((toupper("$GH_AMBUSH") = "ON") && ($hidden != 1)) then
	{
		...
		elseif (($Stealth.LearningRate < 33)) then gosub HIDE
	}
	if ((toupper("$GH_HUNT") = "ON") && (%t >= %HUNT_TIME) && ($Perception.LearningRate < 34)) then
	{
		put hunt
		var HUNT_TIME %t
	...
	if ((toupper("$GH_MANIP") = "ON") && (%t >= %MANIP_TIME) && ($Empathy.LearningRate < 34) then
	{
		gosub MANIPULATE1
		var MANIP_TIME %t
	...
	if ((toupper("$GH_PP") = "ON") && (%t >= %PP_TIME) && ($Attunement.LearningRate < 34)) then
	{
		send power
		var PP_TIME %t
		math PP_TIME add 60
	...
	if toupper("$GH_BUFF") = "ON" and $mana > 80 then gosub GH_BUFF_ON
...
ATTACK:
var LAST ATTACK
if ($Tactics.LearningRate < 10 && $GH_TACTICS = ON && !matchre("%WEAPON_EXP", "Bow|Crossbow|Slings")) then gosub TACTICSTRAIN
if $GH_ANALYZE = ON then gosub DO_ANALYZE
else
	{
...
TACTICSTRAIN:
        matchre NEXT_TACT Roundtime|You aren't close enough
...
DO_ANALYZE:
	...
	matchre DO_ANALYZE fail to find
	matchre FACE ^Analyze what
	action (combos) on
	put analyze
	matchwait 10
ANALYZEDONE:
	...
	goto COMBOSTART
ANALYZE_FINAL:
	if %ANALYZELEVEL = %ANALYZEAMOUNT then
	{
		put analyze
		pause 1
	}
	gosub maneuver %COMBO(%this.attack)
	math this.attack add 1
	var ANALYZELEVEL 0
COMBOSTART:
	if %this.attack > %combocount then
	{
		var this.attack 1
		return
	}
	if ((%this.attack = %combocount) && (%ANALYZELEVEL < %ANALYZEAMOUNT)) then goto ANALYZE_FINAL
	gosub maneuver %COMBO(%this.attack)
	math this.attack add 1
	goto ComboStart
MANEUVER:
...
TMPARALYSIS:
	...
	matchre TMPARALYSIS_END ^I could not find what you were referring to
	matchwait 2
TMPARALYSIS_CAST:
...
SWITCH_WEAPON:
	...
	if (toupper("$GH_MULTI") != "OFF") then
	{
			...
			else put .hunt MULTIWEAPON $GH_MULTI_$GH_MULTI_CURR_NUM
	} else goto DONE
