...
#	math keywords add 1;put #echo >log Orange *** KEYWORD: %keyword_was ****;if %keywords < 0 then math keywords
	...
	action math keywords subtract 2 when ^The (Dwarven|Human) driver says, "I'm leaving shortly
	action math keywords subtract 1 when ^The mule-drawn wagon's driver shouts
	action math keywords subtract 1 when ^The horse-drawn carriage's driver shouts,
	action math keywords subtract 1 when ^You have no idea what you're doing
	action math keywords subtract 1 when ^Stow what\?
	action math keywords subtract 1 when Sayaph
	action math keywords subtract 1 when Sayomi
	action math keywords subtract 1 when ^Ladar .+ says
	## ** Steal checks
	action math keywords subtract 1 when shakes his head and says,
	action math keywords subtract 1 when shouts, "The (Elothean|Gnome)'s a thief
	action math keywords subtract 1 when begins to shout loudly for help
	action math keywords subtract 1 when says, "I have no need for that\."
	action math keywords subtract 1 when PLEAD
	action math keywords subtract 1 when INNOCENT
	action math keywords subtract 1 when GUILTY
	action math keywords set 0 when ^The door slams shut|^The guard brings you to the jail
	## ** end steal checks
	action math keywords subtract 1 when ^You feel that your gods have smiled upon
	action math keywords subtract 1 when ^You think upon the immortals
	action math keywords subtract 1 when The apprentice (smiles|nods)
	action math keywords subtract 1 when ^Doomsayer
	action math keywords subtract 1 when ^The clerk pages through her ledger
	action math keywords subtract 2 when ^You turn your book to chapter
	action math keywords subtract 1 when RELEASE
	action math keywords subtract 1 when HELP
	action math keywords subtract 2 when System Announcement: .+ Droughtman
	action math keywords subtract 1 when full-voiced
	action math keywords subtract 1 when gives you a raspberry -- "Thbtbtbt!"
	action math keywords subtract 1 when [Y|y]our voice
	action math keywords subtract 1 when spidersilk backpack
	action math keywords subtract 1 when ^A giant blade spider
	action math keywords subtract 1 when ^The blade spider
	action math keywords subtract 1 when ^You are engaged to a giant blade spider
	action math keywords subtract 1 when ^With a quick breath, you begin to sing the opening notes to "Merelew's Legacy", keeping each phrase light and airy like the passage of the wind over the sea.
	action math keywords subtract 1 when Table Waiter
	action math keywords subtract 1 when continues to braid
	action math keywords subtract 1 when continues to practice
	action math keywords subtract 1 when continues to hum
	action math keywords subtract 1 when continues to chant
	action math keywords subtract 1 when  frog-crotch bolt
	action math keywords subtract 1 when ^Your mind hears .* thinking, \".*\"
	...
	action math keywords subtract 1 when there are voices coming from that direction
	action math keywords subtract 1 when ^As you begin to sing, a gentle
	action math keywords subtract 1 when ^Glimmering cracks of silver spiderweb
	action math keywords subtract 1 when a ship's rat
	action math keywords subtract 1 when The sentinel stares intently into the shadows,
	action math keywords subtract 1 when lets loose a raspberry
	action math keywords subtract 1 when ^A .+ forest gryphon fans its wings
	action math keywords subtract 1 when ^Voices can be heard over the crashing of the waves, male voices mostly
	action math keywords set 0 when ^EXP HELP for more information|\*\*\*\*IMPORTANT\!\*\*\*\*
	action math keywords set 0 when SHADOWLING|SCROLL {SPELL}
	action math keywords set 0 when ^Judging by the heavens above
	action math keywords set 0 when \[Do you really want to destroy the|^You grab one side of the crucible and pour
	action math keywords set 0 when ^The crucible's contents are not hot enough to empty
	action math keywords set 0 when ^You are certain that the .+ is worth exactly
	action math keywords set 0 when ^You can use SPELL STANCE
	return
