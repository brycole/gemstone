#debuglevel 5
#########################################################################################################
#  ULTIMATE GENIE3 PROGRESSIVE STEALING SCRIPT v5.4 for Dragonrealms. Updated 7/10/15
#  By Shroom of TF / Azothy of Prime. With code contributions from Pelic/Rubinium.
...
#
# - REQUIRES modified travel script. LATEST VERSION HERE - http://pastebin.com/tVwTjDQP )
# - REQUIRES ExpTracker Plugin (Find on the Genie forums)
# - MAY require using my modified Automapper script (IF you have problems with the script moving properly) - http://pastebin.com/wx1cM7ty
# - (OPTIONAL) Uses Time Plugin
#
# - USAGE - Simply start script with .steal
# - For Troubleshooting/testing - .steal <label to skip to> <type anything to SKIP BAG CHECK>
# - For example - .steal CROSS.TO.CARD YES will skip directly to CROSS.TO.CARD label and skip the bag check and turn on debug mode
# - This is for testing/troubleshooting purposes only, usually after already starting stealing script and you want to restart it halfway through. 
# - Skips the bag check so it doesn't put already stolen items in your safe container
# - This is assuming you already went through the beginning bag check - USE AT YOUR OWN RISK!
# #######################################################################################################
...
# ~~ I TEST THIS SCRIPT WITH MULTIPLE SUPER RARE ITEMS~ I HAVE ONLY EVER LOST ONE ITEM IN THOUSANDS OF RUNS,
...
## Script will automatically scan your bags and stash any vulnerable items in your safe container
## This is just a double layer of protection!
...
# MARK.ALL ON TO MARK EVERY ITEM YOU STEAL.  
# MARK.ALL OFF TO SKIP THE SECOND AND FOURTH MARK (USEFUL FOR LOW LEVEL THIEVES)
# STEALCOUNT IS THE MAX NUMBER OF TIMES YOU WILL STEAL FROM A SHOP - REGARDLESS OF MARK! 5 IS THE ABSOLUTE MAX!
# BE WARNED STEALCOUNT WILL OVERRIDE YOUR MARK COUNT, SO IF YOU STOP MARKING ON THE SECOND MARK DUE TO DRAWING ATTENTION, IT WILL STILL STEAL THE MAX NUMBER 
	var MARK.ALL ON
     var STEALCOUNT 5
...
###############################
## EOTB IS PRETTY MUCH MANDATORY FOR NECROS
## ROC IS OPTIONAL IF YOU THINK YOU HAVE ENOUGH MANA FOR IT
     var EOTB ON
	var eotbMana 20
     var ROC ON
	var rocMana 7
...
###############################
## POWER IS ONLY FOR NOOBIE EMPATHS, IN PLACE OF PERCEIVE HEALTH
## IF YOU ARE A HIGHER LEVEL EMPATH, YOU CAN LEAVE POWER OFF AND TURN PERC.HEALTH ON. 
     var POWER OFF
     var PERC.HEALTH OFF
...
ACTION.INIT:
	...
	action instant goto JAIL.CHECK when your belongings have been stripped|in a jail cell wearing a set of heavy manacles|strip you of all your possessions|binds your hands behind your back
     ...
     action var no.learn ON; put #echo >log Orange *** Not learning from %item in %SHOP @ $Thievery.Ranks ranks when You don't feel you learned anything useful
...
######################################################################################################################################
     var STEAL.LIST %STEAL.LIST.THEREN|%STEAL.LIST.HAVEN|%STEAL.LIST.ARTHE|%STEAL.LIST.CROSS|%STEAL.LIST.LETH|%STEAL.LIST.SHARD|%STEAL.LIST.RATHA|%STEAL.LIST.AESRY|%STEAL.LIST.MISC
...
CROSSING.EMPATH3:
     ...
     var SECOND.ITEM 1
     var item.Location %C.Empath3.Loc
...
LETH.EXP.CHECK.5:
     if $Thievery.LearningRate > 33 then goto DONE.STEALING.LETH
     if $Thievery.Ranks >= 465 then goto LETH.EXP.CHECK.6
LETH.TO.PERFUME:
...
LETH.DECIDE:
     if ("%SINGLE.RUN" = "ON") then goto PAWN.LETH
     if $Thievery.LearningRate > 30 then goto PAWN.LETH
...
WARRANT.CHECK.SHARD:
     ...
     pause 0.5
     gosub TIME.CHECK
	if ("%TIME" = "NIGHT") then put #echo >Log Yellow *** Night time! Many shops closed. Reduced stealing. Doh. 
	gosub COOKIE
	if ("%guild" = "Necromancer") then gosub NECRO.PREP
SHARD.TO.GENERAL:
     var LAST SHARD.TO.GENERAL
     gosub AUTOMOVE general
SHARD.GENERAL:
     var shopcode S.Gen
...
SHARD.HERALDRY:
     ...
     var item.Location %S.Herald.Loc
...
HIB.ALCHEMY:
     ...
     gosub STEAL %H.Alch
HIB.EXP.CHECK.2:
     if $Thievery.Ranks >= 430 then goto SURRENDER.HIB
HIB.TO.FORGE:
     var LAST HIB.TO.FORGE
     gosub AUTOMOVE forge
HIB.FORGE:
     var shopcode H.Forge
     var SHOP Hib Forge
     var item.Location %H.Forge.Loc
     gosub STEAL %H.Forge
SURRENDER.HIB:
     var LAST LEAVE.HIB
     gosub AUTOMOVE 200
...
RATHA.ORIGAMI:
     ...
     var item.Location %R.Origami.LOC
...
FINISHED.SCRIPT:
	...
	put #echo >Log Lime * Total Time: %runTime minutes
     ...
     put #echo >Log Lime * Total Profit: %Plat Plat, %Gold Gold, %Silver Silver, %Bronze Bronze and %Copper Copper
	echo **** STEALING REPORT ****
	echo **** Thievery Rank: $Thievery.Ranks $Thievery.LearningRate/34
	...
	echo **** Total Profit: %Plat Plat, %Gold Gold, %Silver Silver, %Bronze Bronze and %Copper Copper
     pause 6
...
TRAVEL.HOME:
     ...
     if "$zoneid" = "42" then gosub AUTOMOVE portal
...
MARKING:
     ...
     pause 0.1
	pause 0.1
     matchre MARKING ^\.\.\.wait\s+\d+\s+sec(?:onds|s)?\.?|^Sorry\,
...
STEALING:
	...
	if !$hidden then gosub HIDE
     ...
     pause 0.1
	pause 0.1
     matchre STEALING ^\.\.\.wait\s+\d+\s+sec(?:onds|s)?\.?|^Sorry\,
     ...
     send steal %item %item.Location
     matchwait
STEAL.CHECK:
     if ("%no.learn" = "ON") then goto STEAL.RETURN
...
PAWN.GET:
     pause 0.001
     pause 0.001
     matchre PAWN.GET ^\.\.\.wait\s+\d+\s+sec(?:onds|s)?\.?|^Sorry\,
...
PAWN.SELL:
	...
	if ((matchre("%nosell", "$righthand")) || (matchre("%nosell", "$lefthand")) then goto PAWN.BIN
     matchre PAWN.SELL ^\.\.\.wait\s+\d+\s+sec(?:onds|s)?\.?|^Sorry\,
     matchre PAWN.GET ^You sell your
     matchre PAWN.BIN slit a throat for this|isn't worth my time
     matchre BUCKET You'll want to empty that first
     send sell my %PAWN.ITEM
     matchwait 5
PAWN.BIN:
     if ("%BIN" = "OFF") then goto BUCKET
     matchre PAWN.BIN ^\.\.\.wait\s+\d+\s+sec(?:onds|s)?\.?|^Sorry\,
...
BUCKET:
	if "$zoneid" = "30" then 
     ...
     pause 0.1
     matchre BUCKET ^\.\.\.wait\s+\d+\s+sec(?:onds|s)?\.?|^Sorry\,
     matchre PAWN.GET ^You put
...
BIN.GET:
     pause 0.001
     matchre BIN.GET ^\.\.\.wait\s+\d+\s+sec(?:onds|s)?\.?|^Sorry\,
     ...
     matchwait 5
BIN.ITEM:
     matchre BIN.ITEM ^\.\.\.wait\s+\d+\s+sec(?:onds|s)?\.?|^Sorry\,
...
STOW:
	...
	if matchre("%item","(%BIN.CONTAINER|%PAWN.CONTAINER)") then gosub EMPTY.HANDS
     ...
     matchre STOW ^\.\.\.wait\s+\d+\s+sec(?:onds|s)?\.?|^Sorry\,
     ...
     if "$righthand" != "Empty" then send put my $righthandnoun in my %BIN.CONTAINER
     pause 0.1
     if "$lefthand" != "Empty" then send put my $lefthandnoun in my %PAWN.CONTAINER
     matchwait 7
     RETURN
SECOND.BAG:
     pause 0.001
     matchre SECOND.BAG ^\.\.\.wait\s+\d+\s+sec(?:onds|s)?\.?|^Sorry\,
     ...
     if "$righthand" != "Empty" then send put my $righthandnoun in my %PAWN.CONTAINER
     if "$lefthand" != "Empty" then send put my $lefthandnoun in my %BIN.CONTAINER
     matchwait 7
     RETURN
STOW.RETURN:
     var STOW.LOOP 0
...
STOW.BIN:
	...
	pause 0.001
     ...
     if "$righthand" != "Empty" then send put my $righthandnoun in my %SAFE.CONTAINER
     if "$lefthand" != "Empty" then send put my $lefthandnoun in my %SAFE.CONTAINER
     matchwait 7
STOW.BIN.RETURN:
	pause 0.001
...
REMOVE.ARMOR:
	...
	pause 0.1
     ...
     matchre REMOVE.ARMOR ^\.\.\.wait\s+\d+\s+sec(?:onds|s)?\.?|^Sorry\,
...
STOW.ARMOR:
	...
	pause 0.1
     var LAST STOW.ARMOR
     matchre STOW.ARMOR ^\.\.\.wait|^Sorry, you may only type
...
COOKIE.START.CONTINUE:
     ...
     matchre COOKIE.START.CONTINUE ^\.\.\.wait\s+\d+\s+sec(?:onds?|s)?\.?$|^Sorry\,
...
GUARDIAN.SPIRIT:
     ...
     if "$zoneid" = "42" then RETURN
      ...
      send prep gs
      pause 20
      matchre GS1 ^You're not ready
      matchre END.GS ^You sense
      send cast
      pause
      pause 0.5
      RETURN
...
#Waits for invis pulse or casts the spell if invisible is off.
	if (("%Eyes" = "ON") && ($invisible = 0)) then 
		{
			...
			matchwait 30
			put #log >Log $time
			put #log >Log EOTB match timed out.
		}
	...
	if (%castPause > 0) then goto SPELL_WAIT
	else pause %castPause
	gosub SPELL_CAST
...
BANK:
	send wealth
	waitforre ^Wealth|^I could|^\.\.\.wait\s+\d+\s+sec(?:onds?|s)?\.?$|^Sorry\,
	pause 0.001
...
COMBINE.BANK:
	math Plat add $Plat
	math Gold add $Gold
	math Silver add $Silver
	math Bronze add $Bronze
	math Copper add $Copper
	RETURN
#############################################
## EXCHANGE
#############################################
...
HIB.JAIL.RETURN:
     ...
     gosub AUTOMOVE sentin
CLAN:
     if ("$gamename" = "DRF") && ("%CLANITEM" = "YES") then goto CLAN_ITEM
     else goto THE_SACK
CLAN_ITEM_PAUSE:
     pause 8
CLAN_ITEM:
...
PUT:
     ...
     matchre RETURN_CLEAR ^You (?:hand|push|move|put|tap|drop|place|toss|set|add|search|circle|fake|slip|weave|shove|try|must|wave|sit|fail|turn|are already|can't|glance|bend|kneel|carefully|quietly|sense|begin|rub|sprinkle) .*(?:\.|\!|\?)
...
PUT_IT:
     ...
     matchre WEBBED ^You can't do that while entangled in a web
     matchre STUNNED ^You are still stunned
     matchre RETURN ^You (?:put|drop|carefully) .*\.
     matchre RETURN ^Please rephrase that command\.
     matchre RETURN ^.* what\?
     matchre RETURN ^I could not find what you were referring to\.
     matchre RETURN ^What were you referring to\?
...
HELP:
     echo *********************************************************************************************************************
     echo *** Usage is .steal <1> <2> <3> 1- Label you want to skip to, 2- Starting City - 3- Type anything to skip bag check
     echo *** Variables useful for debugging purposes, or for re-starting the script from a certain point.
     echo *** otherwise just start script with NO variables ( . steal )
     echo **********************************************************************************************************************
     exit