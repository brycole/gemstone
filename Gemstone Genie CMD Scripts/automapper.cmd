...
action goto invischute when ^You just can't hide and plunge
action var removeice 1; goto iceskates when ^Your movement is hindered somewhat by your ice skates|^Your movement is greatly hindered
action var wearice 1; goto iceskates when ^You had better slow down! The ice is far too treacherous|^You had better slow down! The ice is far too treacherous
...
stand.loop:	
	if (($standing != 1) && ("$zoneid" != "42") && ("$roomid" != "204")) then
	{
		put dance silly
		pause
		goto stand.loop
...
move.wait:
	...
	goto move.done	
move.stand:
	if (($standing != 1) && ("$zoneid" != "42") && ("$roomid" != "204")) then goto return.clear 
	action (mapper) off
	pause .5
